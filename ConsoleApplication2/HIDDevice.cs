			...
			if ( m_hHandle != InvalidHandleValue )	// if the open worked...
			{
				...
				if (HidD_GetPreparsedData(m_hHandle, out lpData))	// get windows to read the device data into an internal buffer
				{
					try
					{
                        ...
                        SafeFileHandle handl = new SafeFileHandle(m_hHandle, true);
						m_oFile = new FileStream(handl, FileAccess.Read | FileAccess.Write, m_nInputReportLength, true);	// wrap the file handle in a .Net file stream
